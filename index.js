...
const order_model = require('./orderModel')
app.use(express.json())
app.use(function (req, res, next) {
  res.setHeader('Access-Control-Allow-Origin', 'https://database-frontend.vercel.app/');
...
app.post('/orders', (req, res) => {
  order_model.createOrder(req.body)
  .then(response => {
    res.status(200).send(response);
  })
  .catch(error => {
...
app.post('/status', (req, res) => {
  order_model.getStatus(req.body)
  .then(response => {
    res.status(200).send(response);
  })
  .catch(error => {
